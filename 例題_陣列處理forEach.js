...
data.forEach(function(item){      // 這裡把利用forEach取出來的值的名稱稱為item
    if (item.Charge == '免費'){   // 檢查每一筆item中的charge屬性要符合'免費'，才是true
        chargeSpot.free += 1;
    }
    else{
        chargeSpot.paid += 1;
...
console.log(`免費充電站共有 ${chargeSpot.free} 處，付費充電站共有 ${chargeSpot.paid} 處。`);

// console.log(data.Charge); 如果if的條件是設定「data.Charge」，會因為
// data 是一個陣列資料，陣列中的元素才是物件，物件中才有Charge屬性而會印出 undefined。
// 若要抓到Charge屬性的值是免費或投幣式，則須設定成利用forEach抓取data這個陣列資料中的item(第一個參數，是每個元素的值)，
// item(這個值是一個物件)，則需抓取這個物件的Charge屬性，所以if的條件必須設定為 item.Charge，才能確實抓到免費或投幣式進行判斷。


// 若要把抓取出來的免費或付費的充電站的詳細資料也分別列出來，
// 可各設定免費或付費兩個空陣列，若判斷完是免費或付費的item(物件)再push回去空陣列。
// 印出免費或付費兩個陣列即可獲得免費或付費充電站的詳細資訊。
...
data.forEach(function(item){
    if (item.Charge == '免費'){ 
        chargeSpotList.free.push(item);
    }
    else{
        chargeSpotList.paid.push(item);
    }
})

console.log (chargeSpotList.free);
console.log (chargeSpotList.paid);